...
def muon_step_fused(
    ...
    """
    Fused Muon step: momentum -> polar_express -> update
    All in one compiled graph to eliminate Python overhead between ops.
...
class MuonAdamW(torch.optim.Optimizer):
    ...
    Arguments:
        param_groups: List of dicts, each containing:
            ...
            - For Muon groups: 'lr', 'momentum', 'ns_steps', 'weight_decay'
    """
    ...
    def _step_muon(self, group: dict) -> None:
        ...
        metrics_grad = stacked_grads.detach().clone() if capture_metrics else None
        ...
        # Single fused kernel: momentum -> polar_express -> update
        muon_step_fused(
            stacked_grads,
            stacked_params,
            momentum_buffer,
        ...
        if capture_metrics and metrics_grad is not None:
            mom = float(group["momentum"])
            m_tilde = metrics_grad.clone()
            m_tilde.lerp_(momentum_buffer.detach(), mom)
            state["metrics_cache"] = {
                "optimizer_kind": "native_muon",
                "param_start_idx": 0,
                "num_owned": num_params,
                "grad": metrics_grad.detach(),
                "momentum": momentum_buffer.detach().clone(),
                "momentum_after_nesterov": m_tilde.detach(),
                "sigma": None,
            }
...
class DistMuonAdamW(torch.optim.Optimizer):
    ...
    def _compute_muon(self, group: dict, info: dict, gather_list: list, rank: int) -> None:
        ...
        if num_owned > 0:
            ...
            capture_metrics = bool(group.get("_capture_metrics", False))
            metrics_grad = grad_chunk[:num_owned].detach().clone() if capture_metrics else None
            ...
            if capture_metrics and metrics_grad is not None:
                mom = float(group["momentum"])
                momentum_owned = state["momentum_buffer"][:num_owned].detach()
                m_tilde = metrics_grad.clone()
                m_tilde.lerp_(momentum_owned, mom)
                state["metrics_cache"] = {
                    "optimizer_kind": "native_muon",
                    "param_start_idx": start_idx,
                    "num_owned": num_owned,
                    "grad": metrics_grad.detach(),
                    "momentum": momentum_owned.clone(),
                    "momentum_after_nesterov": m_tilde.detach(),
                    "sigma": None,
                }
            updated_params[:num_owned].copy_(stacked_owned)
        elif group.get("_capture_metrics", False):
            state = self.state[p]
            state["metrics_cache"] = {
                "optimizer_kind": "native_muon",
                "param_start_idx": start_idx,
