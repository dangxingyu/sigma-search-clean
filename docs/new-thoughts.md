...
- Reference structured configs should fix SOAP at `beta1=0.95`, `beta2=0.99`, and `shampoo_beta=0.95`; for handoff it now uses `precondition_frequency=1` to remove frequency as an extra confound against KL-SOAP. KL-Shampoo also uses `precondition_frequency=1` with the prototype-style beta recipe (`beta1=0.9`, `beta2/shampoo_beta=0.98`), while ordinary Shampoo remains opt-in at `precondition_frequency=10`; KL-SOAP now uses `beta2=0.95`, `shampoo_beta=0.9`, `precondition_frequency=1`, and `init_factor=0.1`.
