...
- Reference structured configs should fix SOAP at `beta1=0.95`, `beta2=0.99`, and `shampoo_beta=0.95`; for handoff it now uses `precondition_frequency=1` to remove frequency as an extra confound against KL-SOAP. KL-Shampoo also uses `precondition_frequency=1` with the prototype-style beta recipe (`beta1=0.9`, `beta2/shampoo_beta=0.98`), while ordinary Shampoo remains opt-in at `precondition_frequency=10`; PR-290 KL-SOAP-H uses `beta2=0.9`, `shampoo_beta=0.9`, `precondition_frequency=1`, and `init_factor=0.1`.
- For handoff baseline sweeps, `plain_muon` should be a fixed alpha=`1.0` best-LR control, not another LR-swept method. This keeps the baseline comparison focused on AdamW/KL-Shampoo quality and avoids burning compute redoing the already-completed identity LR sweep.
...
- The follow-up Fantastic/PR-aligned d8 tune confirms the KL-SOAP-over-AdamW result: KL-SOAP `1.523088 @ lr=0.008` vs AdamW `1.677638 @ lr=0.0005`.
...
- The updated SOAP/Shampoo/KL-Shampoo tune still fails to beat AdamW. SOAP best is `1.726418`, Shampoo best is `1.741547`, and KL-Shampoo remains flat/poor around `2.353`. This suggests KL-SOAP is the only currently credible structured baseline among these four, while KL-Shampoo likely has either a formulation mismatch or implementation bug.
