...
Latest update:
- 2026-05-16 frequency handoff update: changed auto configs for `soap` and `kl_shampoo` to `precondition_frequency=1`, matching the KL-SOAP refresh cadence for the handoff baseline comparison. Ordinary `shampoo` remains implemented but opt-in with `precondition_frequency=10`.
- 2026-05-16 handoff simplification: changed default optimizer-baseline groups to `plain_muon adamw soap kl_soap kl_shampoo`. `plain_muon` no longer runs an LR grid by default; it runs exactly one LR per requested d12/d16 batch, copied from the previous alpha=`1.0` best rows: d12 `{512K:0.0075, 2M:0.015, 8M:0.01}` and d16 `{512K:0.005, 2M:0.005, 8M:0.015}`. Ordinary `shampoo` remains implemented but opt-in.
- 2026-05-16 config handoff update: changed ordinary `shampoo` auto config to use the same beta recipe as `kl_shampoo`: `beta1=0.9`, `beta2/shampoo_beta=0.98`, `precondition_frequency=10`, `structured_init_factor=0.1`. This is a handoff simplification and plausible lower-smoothing choice, not a swept beta optimum; KL-Shampoo's `0.98` beta was reference/prototype-aligned and only LR-validated, not beta-swept.
- 2026-05-16 handoff sweep audit: changed the d12/d16 optimizer-baseline handoff wrappers to method-grouped fixed LR grids with `ADAPTIVE_LR=0` by default, because the target cluster should not depend on adaptive boundary-closure jobs. The grids were adjusted from prior d8 tuning and old d12/d16 LR-shape evidence: plain Muon around `0.02-0.06` with lower d16 guard points, AdamW explicitly including the old key `0.005/0.0075` region while retaining lower-LR guards, KL-SOAP/SOAP around `0.004-0.012`, KL-Shampoo around its ref-patched d8 best `0.008` and left guards, and ordinary Shampoo using old d12/d16 key points `0.005/0.0075/0.01/0.015/0.02` because it was not fully d8-retuned after the reference patch.
- 2026-05-15 handoff sweep audit: corrected the d12/d16 optimizer-baseline handoff wrappers to use method-grouped LR grids instead of forcing AdamW, Muon, SOAP, Shampoo, KL-SOAP, and KL-Shampoo onto one shared grid.
...
Latest update:
- Follow-up audit against `/blue/yanjun.li/xd7812.princeton/optimizer-sft` found another real config mismatch: vanilla SOAP there uses the standard SOAP recipe `beta1/theta=0.95`, `beta2=0.99`, `precondition_frequency=10`, and `shampoo_beta=0.95`. The clean repo auto config had incorrectly set SOAP/Shampoo `shampoo_beta=0.90`.
- Fixed the clean auto config to SOAP/Shampoo `shampoo_beta=0.95`. Also changed non-KL SOAP/Shampoo bootstrap to match the vanilla SOAP reference: initialize preconditioner factors at zero, apply the first EMA update with weight `(1 - shampoo_beta)`, initialize the eigenbasis from that EMA state, and skip the first optimizer update.
- Modal d8 structured baseline bugcheck `modal_d8_baseline_bugcheck_20260512_160047` completed on `8xB200`, depth `8`, batch `262144`, `0.1x` Chinchilla (`153` steps). This run used the old SOAP/Shampoo auto config (`beta2=0.95`, `shampoo_beta=0.95`) and default `weight_decay=0.28`, so keep it as a bugcheck only. It did show KL-SOAP is not fundamentally broken: best KL-SOAP `1.509367 @ lr=0.015`, beating AdamW `1.677759 @ lr=0.0005`; SOAP best `1.704571 @ lr=0.002`, Shampoo best `1.746945 @ lr=0.00025`, KL-Shampoo best `2.353443 @ lr=0.0005`.
- Started then aborted `modal_d8_structured_tune_20260512_194403` before completion because the wrapper default `LR_MIN=0.0005` would have prevented useful low-LR boundary extension for Shampoo when the initial grid included `0.000125`.
- Relaunched follow-up Modal tune `modal_d8_structured_tune_lrmin_20260512_194654` with updated Fantastic/PR-aligned configs, `weight_decay=0.1`, and explicit `LR_MIN=0.00003125`. Grid: depth `8`, batch `262144`, `0.1x` Chinchilla, methods `{adamw, soap, shampoo, kl_soap, kl_shampoo}`, LR `{0.000125,0.00025,0.0005,0.001,0.002,0.004,0.008,0.015,0.03,0.06}`, adaptive LR enabled.
- `modal_d8_structured_tune_lrmin_20260512_194654` completed with `52` rows and `0` errors. Best rows: KL-SOAP `1.523088 @ lr=0.008`, AdamW `1.677638 @ lr=0.0005`, SOAP `1.726418 @ lr=0.004`, Shampoo `1.741547 @ lr=0.000125`, KL-Shampoo `2.353109 @ lr=0.000125`. Conclusion: KL-SOAP beats AdamW in this short d8/262K sanity run; SOAP/Shampoo/KL-Shampoo do not.
...
- Next required result: rerun at least a small d8/d12 sanity grid before interpreting SOAP/Shampoo/KL-SOAP quality. Updated handoff configs are vanilla SOAP `beta1=0.95`, `beta2=0.99`, `shampoo_beta=0.95`, `precondition_frequency=1`; KL-Shampoo `beta1=0.9`, `beta2/shampoo_beta=0.98`, `precondition_frequency=1`, `structured_init_factor=0.1`; ordinary Shampoo opt-in with the same beta recipe but `precondition_frequency=10`; and PR-290 KL-SOAP-H-style `beta1=0.95`, `beta2=0.9`, `shampoo_beta=0.9`, `precondition_frequency=1`, `structured_init_factor=0.1`; baseline wrappers now default `WEIGHT_DECAY=0.1`.
