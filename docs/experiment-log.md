...
Latest update:
- 2026-05-15 handoff sweep audit: corrected the d12/d16 optimizer-baseline handoff wrappers to use method-grouped LR grids instead of forcing AdamW, Muon, SOAP, Shampoo, KL-SOAP, and KL-Shampoo onto one shared grid. The default groups are `plain_muon`, `adamw`, `soap_klsoap`, `kl_shampoo`, and `shampoo`, each with its own LR range and adaptive boundary closure.
