...
Immediate next steps:
- Rerun a small fixed-implementation sanity grid before any full base optimizer sweep. Default handoff should compare `plain_muon`, `adamw`, `soap`, `kl_soap`, and `kl_shampoo`; keep ordinary `shampoo` as opt-in until it has stronger d8 evidence.
- Use reference-aligned configs for structured methods, with one handoff simplification: SOAP keeps the vanilla beta recipe `beta1=0.95`, `beta2=0.99`, `shampoo_beta=0.95` but uses `precondition_frequency=1`; KL-SOAP-style now uses `beta1=0.95`, `beta2=0.95`, `shampoo_beta=0.9`, `precondition_frequency=1`, `structured_init_factor=0.1`; KL-Shampoo uses the prototype-style beta recipe `beta1=0.9`, `beta2=0.98`, `shampoo_beta=0.98`, `precondition_frequency=1`, `structured_init_factor=0.1`; ordinary Shampoo remains opt-in with the same beta recipe as KL-Shampoo but `precondition_frequency=10`.
...
- For optimizer-baseline comparisons, use `scripts/run_d12_optimizer_baselines.sh` and `scripts/run_d16_optimizer_baselines.sh`. They keep the same 2x-Chinchilla `{512K,2M,8M}` model/data/batch recipe. Default groups are `plain_muon adamw soap kl_soap kl_shampoo`; `plain_muon` is not LR-swept and uses the previous alpha=`1.0` best LR per depth/batch. AdamW, SOAP, KL-SOAP, and KL-Shampoo use compact method-specific LR grids. The wrappers default to `ADAPTIVE_LR=0` and `WEIGHT_DECAY=0.1`; override `WEIGHT_DECAY=0.28` only for strict equality with existing Top-Aware sweeps.
