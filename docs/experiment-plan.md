...
- For optimizer-baseline comparisons, use `scripts/run_d12_optimizer_baselines.sh` and `scripts/run_d16_optimizer_baselines.sh`. They keep the same 2x-Chinchilla `{512K,2M,8M}` model/data/batch recipe, but use method-grouped fixed LR grids because AdamW, Muon, SOAP, and Shampoo do not share an LR scale. Defaults are `plain_muon: {0.0025,0.005,0.0075,0.01,0.015,0.02,0.03,0.04,0.06,0.08}`, `adamw: {0.00025,0.0005,0.001,0.002,0.003,0.004,0.005,0.006,0.0075}`, `soap/kl_soap: {0.001,0.002,0.003,0.004,0.005,0.006,0.008,0.012,0.016,0.024}`, `kl_shampoo: {0.001,0.002,0.004,0.005,0.006,0.008,0.012,0.016,0.024}`, and `shampoo: {0.0005,0.001,0.002,0.004,0.005,0.006,0.008,0.012,0.016,0.02,0.024}`. They default to `ADAPTIVE_LR=0` and `WEIGHT_DECAY=0.1`; override `WEIGHT_DECAY=0.28` only for strict equality with existing Top-Aware sweeps.
