...
Immediate next steps:
- Rerun a small fixed-implementation sanity grid before any full base optimizer sweep. Default handoff should compare only `plain_muon`, `adamw`, and `kl_shampoo`; keep `shampoo`, `soap`, and `kl_soap` as opt-in groups until they have stronger d8 evidence.
...
- For optimizer-baseline comparisons, use `scripts/run_d12_optimizer_baselines.sh` and `scripts/run_d16_optimizer_baselines.sh`. They keep the same 2x-Chinchilla `{512K,2M,8M}` model/data/batch recipe. Default groups are `plain_muon adamw kl_shampoo`; `plain_muon` is not LR-swept and uses the previous alpha=`1.0` best LR per depth/batch. AdamW and KL-Shampoo still use compact method-specific LR grids. The wrappers default to `ADAPTIVE_LR=0` and `WEIGHT_DECAY=0.1`; override `WEIGHT_DECAY=0.28` only for strict equality with existing Top-Aware sweeps.
