...
- For optimizer-baseline comparisons, use `scripts/run_d12_optimizer_baselines.sh` and `scripts/run_d16_optimizer_baselines.sh`. They keep the same 2x-Chinchilla `{512K,2M,8M}` model/data/batch recipe, but now use method-grouped LR grids because AdamW, Muon, SOAP, and Shampoo do not share an LR scale. Defaults are `plain_muon: {0.01,0.02,0.03,0.04,0.06,0.08}`, `adamw: {0.00025,0.0005,0.001,0.002,0.004}`, `soap/kl_soap: {0.002,0.004,0.006,0.008,0.012,0.016,0.024}`, `kl_shampoo: {0.002,0.004,0.008,0.015,0.03}`, and `shampoo: {0.0005,0.001,0.002,0.004,0.008}`. They default to `WEIGHT_DECAY=0.1` for the Fantastic-style baseline recipe; override `WEIGHT_DECAY=0.28` only for strict equality with existing Top-Aware sweeps.
